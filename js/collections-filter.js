...
(function () {
  ...
  let closeFilterBtn = null;
  let filterDrawer = null;
  let filterBackdrop = null;
  let drawerBrandList = null;
  let drawerScaleBtns = [];
  let drawerSortSelect = null;
  let drawerResetBtn = null;
  let drawerApplyBtn = null;
  let filterPillsBar = null;

  function reindexCards() {
    grid = document.querySelector('.grid.grid-cols-2') || document.querySelector('.collection-grid');
    if (!grid) return;
    ...
    // Populate or update Drawer Brands
    if (drawerBrandList) {
    ...
    // Populate or update Top Brand Pills Bar
    countContainer = document.querySelector('section div.mb-8') || (grid ? grid.previousElementSibling : null);
    if (!filterPillsBar && countContainer) {
      filterPillsBar = document.getElementById('brand-pills-bar');
      if (!filterPillsBar) {
  ...
  function applyAllFilters(updateUrl = true) {
    if (!grid || cardData.length === 0) return;
    let visibleCount = 0;
    ...
    // Update drawer brand button active states
    if (drawerBrandList) {
  ...
  function initCollections() {
    ...
    filterBackdrop = document.getElementById('filter-drawer-backdrop');
    drawerBrandList = document.getElementById('drawer-brand-list');
    drawerScaleBtns = Array.from(document.querySelectorAll('.filter-scale-btn'));
    drawerSortSelect = document.getElementById('drawer-sort-select');
    drawerResetBtn = document.getElementById('drawer-reset-btn');
    drawerApplyBtn = document.getElementById('drawer-apply-btn');
  ...
  window.reindexCollections = reindexCards;
