...
(function () {
  ...
  function init3DHero() {
    ...
    // Shadow Plane with candidate fallback URLs
    const textureLoader = new THREE.TextureLoader();
    const shadowCandidates = ['/shadow.png', 'shadow.png', './shadow.png', '/public/shadow.png'];
    let shadowCandidateIdx = 0;
    function loadShadow() {
      if (shadowCandidateIdx >= shadowCandidates.length) return;
      const sUrl = shadowCandidates[shadowCandidateIdx++];
      textureLoader.load(
        sUrl,
        shadowTex => {
    ...
    // Load Car Model with multi-URL candidate fallbacks
    let carModel = null;
    ...
    const modelCandidates = [
      ...
      'free_bmw_m3_e30.glb',
      './free_bmw_m3_e30.glb',
      '/public/free_bmw_m3_e30.glb'
    ];
    let modelCandidateIdx = 0;

    function loadModel() {
      if (modelCandidateIdx >= modelCandidates.length) {
        console.error('❌ [3D Hero] Failed to load BMW M3 model from all candidate URLs.');
        return;
      }
      const modelUrl = modelCandidates[modelCandidateIdx++];
      console.log(`[3D Hero] Attempting to load car model from: ${modelUrl}`);
      loader.load(
        modelUrl,
        gltf => {
          carModel = gltf.scene;
          ...
          console.log(`✅ [3D Hero] BMW M3 Model successfully loaded into scene from ${modelUrl}!`);
        },
        undefined,
        err => {
          console.warn(`[3D Hero] Could not load from ${modelUrl}, trying next fallback...`, err);
          loadModel();
        }
      );
    }
